...
import {
    ...
    TrendingUp, Star, Clock, MapPin, Shield, Users, Zap,
...
// Helper Functions
const filterByCategory = (umkms, category) => {
    if (!category) return umkms;
    return umkms.filter(u => u.category?.toLowerCase().includes(category));
};

const searchUmkms = (umkms, term) => {
    if (!term) return umkms;

    const searchTerm = term.toLowerCase().trim();

    return umkms.filter(umkm => {
        const matchName = umkm.name?.toLowerCase().includes(searchTerm);
        const matchCategory = umkm.category?.toLowerCase().includes(searchTerm);
        const matchAddress = umkm.address?.toLowerCase().includes(searchTerm);
        const matchDescription = umkm.description?.toLowerCase().includes(searchTerm);

        const addressParts = umkm.address?.toLowerCase().split(/[,.\-/]/);
        const matchLocationParts = addressParts?.some(part =>
            part.trim().includes(searchTerm)
        );

        return matchName || matchCategory || matchAddress || matchLocationParts || matchDescription;
    });
};
...
// Component
export default function Welcome({ umkms = [], featured = null, canLogin, canRegister, userLocation = null }) {
    ...
    const filteredUmkms = useMemo(() => {
        let filtered = filterByCategory(umkms, selectedCategory);
        filtered = searchUmkms(filtered, searchTerm);
        return filtered;
    ...
    const handleGetLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                // Reload page dengan query parameter lokasi
    ...
    if (isLoading) {
        return (
            <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-gradient-to-br from-primary-600 via-primary-700 to-primary-900">
                <div className="text-center">
                    <div className="mb-6">
                        <div className="inline-flex items-center justify-center w-20 h-20 bg-white/10 backdrop-blur-sm rounded-2xl animate-pulse">
                            <Store className="w-10 h-10 text-white" />
                        </div>
                    </div>
                    <h1 className="mb-2 text-2xl font-bold text-white">BookUMKM</h1>
    ...
    return (
        <PublicLayout canLogin={canLogin} canRegister={canRegister}>
                            ...
                            <div className="flex gap-2">
                                ...
                                <div className="relative flex-1">
                                    ...
                                    <button
                                        ...
                                        className="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-white transition-all duration-300 bg-gradient-to-r from-primary-600 to-primary-700 rounded-full hover:shadow-lg active:scale-95"
                                    >
                            ...
                            <div className="flex gap-2">
                                ...
                                <div className="relative flex-1">
                                    ...
                                    <input
                                        ...
                                        className="w-full py-3 lg:py-4 pl-12 pr-14 text-base font-medium text-gray-900 placeholder-gray-400 transition-all duration-300 border-0 rounded-full shadow-xl bg-white/95 backdrop-blur-xl focus:shadow-2xl focus:outline-none focus:ring-2 focus:ring-primary-500/30"
                                    />
                                    <button
                                        onClick={scrollToUmkmList}
            ...
            <section className="relative pt-8 pb-4 sm:pt-10 sm:pb-6 bg-gray-50 z-10">
                <div className="mx-auto max-w-7xl">
                            ...
                            <div className="flex gap-3 px-3 snap-x snap-mandatory">
                                {CATEGORIES.map((category) => (
                                    <button
                                        ...
                                        className="flex flex-col items-center gap-2 flex-shrink-0 snap-start w-16"
                                    >
                                        {/* Icon Box */}
                    ...
                    {/* Desktop: Grid */}
                    <div className="hidden sm:grid sm:grid-cols-4 lg:grid-cols-7 gap-4 px-4 lg:px-8">
                        {CATEGORIES.map((category) => (
                            <button
                                key={category.id}
                                onClick={() => {
